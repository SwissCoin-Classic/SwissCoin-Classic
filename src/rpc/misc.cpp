...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total swisscoinclassic balance of the wallet\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"swisscoinclassicaddress\"\n"
            "\nReturn information about the given swisscoinclassic address.\n"
            "\nArguments:\n"
            "1. \"swisscoinclassicaddress\"     (string, required) The swisscoinclassic address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"swisscoinclassicaddress\", (string) The swisscoinclassic address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: SwissCoin Classic address and we have full public key:
        CSwissCoinClassicAddress address(ks);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are swisscoinclassic addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) swisscoinclassic address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CSwissCoinClassicAddress address(innerID);
...
UniValue createwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    CSwissCoinClassicAddress address(witscriptid);
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"swisscoinclassicaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"swisscoinclassicaddress\"  (string, required) The swisscoinclassic address to use for the signature.\n"
...
{
    if (type == 2) {
        address = CSwissCoinClassicAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CSwissCoinClassicAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, int> > &addresses)
{
    if (params[0].isStr()) {
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CSwissCoinClassicAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
