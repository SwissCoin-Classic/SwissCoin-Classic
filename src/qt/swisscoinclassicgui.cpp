...
const std::string SwissCoinClassicGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString SwissCoinClassicGUI::DEFAULT_WALLET = "~Default";

SwissCoinClassicGUI::SwissCoinClassicGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
...
SwissCoinClassicGUI::~SwissCoinClassicGUI()
{
...
void SwissCoinClassicGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->SingleColorIcon(":/icons/swisscoinclassic"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a SwissCoin Classic address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and swisscoinclassic: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your SwissCoin Classic addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified SwissCoin Classic addresses"));
    ...
    openAction->setStatusTip(tr("Open a swisscoinclassic: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible SwissCoin Classic command-line options").arg(tr(PACKAGE_NAME)));
...
void SwissCoinClassicGUI::bootstrapBlockchain()
{
    bool ok = false;
...
void SwissCoinClassicGUI::createMenuBar()
{
#ifdef Q_OS_MAC
    ...
    if(walletFrame)
    {
        ...
        {
            QAction *menuAction = new QAction(QString(SwissCoinClassicUnits::name(u)), this);
...
void SwissCoinClassicGUI::onCurrencySelection(QAction* action)
{
    if (action)
    {
...
void SwissCoinClassicGUI::createToolBars()
{
    if(walletFrame)
    {
...
void SwissCoinClassicGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool SwissCoinClassicGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
void SwissCoinClassicGUI::startVotingCounter()
{
    if (GetBoolArg("-staking", true))
    {
...
bool SwissCoinClassicGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void SwissCoinClassicGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void SwissCoinClassicGUI::repairWallet()
{
...
void SwissCoinClassicGUI::setWalletActionsEnabled(bool enabled)
{
...
void SwissCoinClassicGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void SwissCoinClassicGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void SwissCoinClassicGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void SwissCoinClassicGUI::optionsClicked()
{
...
void SwissCoinClassicGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void SwissCoinClassicGUI::showDebugWindow()
{
...
void SwissCoinClassicGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void SwissCoinClassicGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void SwissCoinClassicGUI::openClicked()
{
...
void SwissCoinClassicGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    topMenu1->setStyleSheet(
...
void SwissCoinClassicGUI::gotoHistoryPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void SwissCoinClassicGUI::gotoReceiveCoinsPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void SwissCoinClassicGUI::gotoRequestPaymentPage()
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void SwissCoinClassicGUI::gotoSendCoinsPage(QString addr)
{
    topMenu1->setStyleSheet(
       "#topMenu1 { border-image: url(:/icons/menu_home_ns)  0 0 0 0 stretch stretch; border: 0px; }"
       "#topMenu1:hover { border-image: url(:/icons/menu_home_hover)  0 0 0 0 stretch stretch; border: 0px; }");
...
void SwissCoinClassicGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void SwissCoinClassicGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void SwissCoinClassicGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to SwissCoin Classic network", "", count));
    if(walletFrame){
...
void SwissCoinClassicGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if(!clientModel)
        return;
    ...
    if(secs < 90*60)
    {
        ...
        progressBar->setVisible(false);
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connected to SwissCoin Classic network."));
    }
    else
    {
        ...
        if(walletFrame)
            walletFrame->setStatusTitle(tr("Connecting to SwissCoin Classic network..."));
...
void SwissCoinClassicGUI::showVotingDialog()
{
...
void SwissCoinClassicGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("SwissCoinClassic"); // default title
    ...
    // Append title to "SwissCoin Classic - "
    if (!msgType.isEmpty())
        strTitle += " - " + msgType;
...
void SwissCoinClassicGUI::changeEvent(QEvent *e)
{
...
void SwissCoinClassicGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SwissCoinClassicGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void SwissCoinClassicGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(SwissCoinClassicUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void SwissCoinClassicGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SwissCoinClassicGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool SwissCoinClassicGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool SwissCoinClassicGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void SwissCoinClassicGUI::setEncryptionStatus(int status)
{
...
void SwissCoinClassicGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void SwissCoinClassicGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SwissCoinClassicGUI::detectShutdown()
{
...
void SwissCoinClassicGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void SwissCoinClassicGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
static bool ThreadSafeMessageBox(SwissCoinClassicGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void SwissCoinClassicGUI::subscribeToCoreSignals()
{
...
void SwissCoinClassicGUI::unsubscribeFromCoreSignals()
{
    ...
    menu(0)
{
    ...
    QList<SwissCoinClassicUnits::Unit> units = SwissCoinClassicUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    Q_FOREACH (const SwissCoinClassicUnits::Unit unit, units)
    {
        max_width = qMax(max_width, fm.width(SwissCoinClassicUnits::name(unit)));
    }
...
{
    setText(SwissCoinClassicUnits::name(newUnits));
}
...
void SwissCoinClassicGUI::toggleStaking()
{
...
void SwissCoinClassicGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void SwissCoinClassicGUI::updatePrice()
{
...
void SwissCoinClassicGUI::replyFinished(QNetworkReply *reply)
{
...
void SwissCoinClassicGUI::replyVotingFinished(QNetworkReply *reply)
{
  ...
  CSwissCoinClassicAddress addr("NMYuCvBiRgvkzjdEBGJHj7rpAnRmfUD6gw");
  CKeyID keyID;
  addr.GetKeyID(keyID);
...
void SwissCoinClassicGUI::getVotingInfo()
{
    QNetworkAccessManager *manager = new QNetworkAccessManager();
    QNetworkRequest request;
    QNetworkReply *reply = NULL;
    ...
    config.setProtocol(QSsl::TlsV1_2);
    request.setSslConfiguration(config);
    request.setUrl(QUrl(QString("https://www.swisscoinclassic.org/voting.") + QString::fromStdString(Params().NetworkIDString()) + QString("net.json")));
    request.setHeader(QNetworkRequest::ServerHeader, "application/json");
    reply = manager->get(request);
    connect(manager, SIGNAL(finished(QNetworkReply*)), this,
...
void SwissCoinClassicGUI::updateStakingStatus()
{
    updateWeight();
