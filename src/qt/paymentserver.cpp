...
const int SWISSCOINCLASSIC_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString SWISSCOINCLASSIC_IPC_PREFIX("swisscoinclassic:");
// BIP70 payment protocol messages
const char* BIP70_MESSAGE_PAYMENTACK = "PaymentACK";
const char* BIP70_MESSAGE_PAYMENTREQUEST = "PaymentRequest";
const char* NIP01_MESSAGE_SIGNEDMSG = "SignedMsg";
// BIP71 payment protocol media types
const char* BIP71_MIMETYPE_PAYMENT = "application/swisscoinclassic-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/swisscoinclassic-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/swisscoinclassic-paymentrequest";
const char* NIP01_MIMETYPE_SIGNEDMSG = "application/swisscoinclassic-signedmsg";
...
{
    QString name("SwissCoinClassicQt");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        // If the swisscoinclassic: URI contains a payment request, we are not able to detect the
        // network as that would require fetching and parsing the payment request.
        // That means clicking such an URI which contains a testnet payment request
        // will start a mainnet instance and throw a "wrong network" error.
        if (arg.startsWith(SWISSCOINCLASSIC_IPC_PREFIX, Qt::CaseInsensitive)) // swisscoinclassic: URI
        {
            savedPaymentRequests.append(arg);

            SendCoinsRecipient r;
            if (GUIUtil::parseSwissCoinClassicURI(arg, &r) && !r.address.isEmpty())
            {
                CSwissCoinClassicAddress address(r.address.toStdString());
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    Q_FOREACH (const QString& r, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(SWISSCOINCLASSIC_IPC_CONNECT_TIMEOUT))
        {
            delete socket;
            socket = NULL;
            return false;
        ...
        socket->write(block);
        socket->flush();
        socket->waitForBytesWritten(SWISSCOINCLASSIC_IPC_CONNECT_TIMEOUT);
    ...
    model(0)
{
    ...
    // on Mac: sent when you click swisscoinclassic: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            LogPrintf("Cannot start swisscoinclassic: click-to-pay handler\n");
        }
        else {
...
//
// OSX-specific way of handling swisscoinclassic: URIs and PaymentRequest mime types.
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in swisscoinclassic: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(SWISSCOINCLASSIC_IPC_PREFIX, Qt::CaseInsensitive)) // swisscoinclassic: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        if (uri.hasQueryItem("m") && uri.hasQueryItem("a")) // sign message URI
        {
            ...
            CSwissCoinClassicAddress addr(uri.queryItemValue("a").toStdString());
            if (!addr.IsValid())
            {
            ...
            QUrl fetchUrlConstructed(s.mid(SWISSCOINCLASSIC_IPC_PREFIX.length()));
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseSwissCoinClassicURI(s, &recipient))
            {
              std::string address_str = recipient.address.toStdString();
              utils::DNSResolver* DNS = nullptr;

              // Validate the passed SwissCoin Classic address
              ...
              if (!address.IsValid()) {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid SwissCoin Classic address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
...
bool PaymentServer::processPaymentRequest(const PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    Q_FOREACH(const PAIRTYPE(CScript, CAmount)& sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            addresses.append(QString::fromStdString(CSwissCoinClassicAddress(dest).ToString()));
        }
        else if (!recipient.authenticatedMerchant.isEmpty()) {
            // Unauthenticated payment requests to custom swisscoinclassic addresses are not supported
        ...
        // SwissCoin Classic amounts are stored as (optional) uint64 in the protobuf messages (see paymentrequest.proto),
        ...
        if (txOut.IsDust(::minRelayTxFee)) {
            Q_EMIT message(tr("Payment request error"), tr("Requested payment amount of %1 is too small (considered dust).")
                .arg(SwissCoinClassicUnits::formatWithUnit(optionsModel->getDisplayUnit(), sendingTo.second)),
                CClientUIInterface::MSG_ERROR);
