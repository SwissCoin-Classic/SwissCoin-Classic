...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"swisscoinclassicprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"swisscoinclassicprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
void ImportAddress(const CSwissCoinClassicAddress& address, const string& strLabel);
void ImportScript(const CScript& script, const string& strLabel, bool isRedeemScript)
{
    ...
    if (isRedeemScript) {
        ...
        ImportAddress(CSwissCoinClassicAddress(CScriptID(script)), strLabel);
    } else {
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid SwissCoin Classic address or script");
    }
...
UniValue importpubkey(const UniValue& params, bool fHelp)
{
    ...
    ImportAddress(CSwissCoinClassicAddress(pubKey.GetID()), strLabel);
...
UniValue importwallet(const UniValue& params, bool fHelp)
{
    ...
    while (file.good()) {
        ...
        if (pwalletMain->HaveKey(keyid)) {
            LogPrintf("Skipping import of %s (key already present)\n", CSwissCoinClassicAddress(keyid).ToString());
            continue;
        }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"swisscoinclassicaddress\"\n"
            "\nReveals the private key corresponding to 'swisscoinclassicaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"swisscoinclassicaddress\"   (string, required) The swisscoinclassic address for the private key\n"
            "\nResult:\n"
    ...
    CSwissCoinClassicAddress address;
    ...
    return CSwissCoinClassicSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by SwissCoin Classic %s\n", CLIENT_BUILD);
    ...
    if (!masterKeyID.IsNull())
    {
        ...
        if (pwalletMain->GetKey(masterKeyID, key))
        {
            ...
            CSwissCoinClassicExtKey b58extkey;
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        std::string strAddr = CSwissCoinClassicAddress(keyid).ToString();
        CKey key;
        if (pwalletMain->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CSwissCoinClassicSecret(key).ToString(), strTime);
