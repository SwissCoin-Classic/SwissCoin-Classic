...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete SwissCoin Classic database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no SwissCoin Classic datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old SwissCoin Classic datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected SwissCoin Classic 0.7 datadir."
    ;;
  3)
    echo "Detected SwissCoin Classic pre-0.8 datadir."
    ;;
  4)
    echo "Detected SwissCoin Classic 0.8 datadir."
    ;;
esac
