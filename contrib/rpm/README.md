...
The RPM spec file provided here is for SwissCoin Classic 0.12.0 and builds on CentOS
...
reference the release tarball available from https://swisscoinclassic.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[SwissCoin Classic git repository](https://github.com/swisscoinclassic/swisscoinclassic) but are not part of
...
Currently only `Source10` is used, for the example `swisscoinclassic.conf` file.
...
in the swisscoinclassic git repository. At present only an SVG version of the SwissCoinClassic
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the swisscoinclassic
...
needed when 0.12.1 is released, a proper fix is already in the SwissCoin Classic git
master branch.
...
idea to build SwissCoin Classic against a static version of that library compiled
...
The `swisscoinclassic.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `swisscoinclassic-server` package. At this point, most RPM based
...
The `swisscoinclassic-server` package should have SELinux support. How to properly do
...
SwissCoin Classic against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/swisscoinclassic/). He has been
packaging SwissCoin Classic for Fedora at least since 2012.
...
4. I renamed the `swisscoinclassic` package that contains the Qt GUI to `swisscoinclassic-core` as
that appears to be how the general population refers to it, in contrast to
`swisscoinclassic-xt` or `swisscoinclassic-classic`. I wanted to make sure the general population
