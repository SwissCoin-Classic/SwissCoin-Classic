...
###   This script attempts to download the signature file SHA256SUMS.asc from swisscoinclassic.org
...
WORKINGDIR="/tmp/swisscoinclassic"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://swisscoinclassic.org/bin/"
VERSIONPREFIX="swisscoinclassic-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'swisscoinclassic-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the SwissCoin Classic binary release signing key installed?"
   fi
